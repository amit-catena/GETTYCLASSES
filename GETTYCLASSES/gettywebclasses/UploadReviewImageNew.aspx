...
.fileUpload{display:inline-block;padding:20px 20px 20px 20px;background: #0192DD url(<%=baseurl%>/images/camera.png) no-repeat 20px 19px; color:#fff; min-width:60px; position:relative; cursor:pointer; font-family:Arial, Helvetica, sans-serif; font-size:14px; text-transform:uppercase;}
...
<form id="ed" onsubmit="return validate(this);" encType="multipart/form-data" method="post" runat="server">
    ...
    <tr><td>Image</td><td><span class="fileUpload">						
					...
					</span> <span style="font-family:Verdana;font-size:11px;color:Red;">please upload image (90 * 45) </span></td> </tr>
		        ...
		        reader.onloadend = function () {
		            ...
		            img.onload = function () {
		                ...
		                if (w > 150 && h > 100) {
		                    ...
		                    cropper = new Darkroom('#target', { minWidth: 200, minHeight: 310, maxWidth: 1300, maxHeight: 600, plugins: { crop: { quickCropKey: 67, minHeight: 45, minWidth: 90} }, init: function () { var cropPlugin = this.getPlugin('crop'); cropPlugin.selectZone(25, 15, 90, 45); }
		                    });
		                    //parent.updateHeight();
		                } else {
